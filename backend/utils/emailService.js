...
const sendEmail = async ({ to, subject, html, text }) => {
  try {
    ...
    const mailOptions = {
      from: {
        name: process.env.BREVO_FROM_NAME || 'Polyibadan Cooperative',
...
const sendVerificationEmail = async (to, code) => {
  return sendEmail({
      ...
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
        ...
        <div style="background: white; padding: 30px; border-radius: 10px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
          ...
          <div style="text-align: center; margin: 30px 0;">
            <div style="background: #f8fafc; border: 2px solid #e2e8f0; border-radius: 8px; padding: 20px; display: inline-block;">
              <h3 style="color: #1e40af; margin: 0; font-size: 24px; letter-spacing: 3px; font-family: 'Courier New', monospace;">${code}</h3>
            </div>
          </div>
    ...
    `,
    text: `Welcome to Polyibadan! Your verification code is: ${code}. This code will expire in 5 minutes.`
  });
};
